...
1. **セキュリティ:**
   - **エスケープ処理:**  
     入力値を安全に処理するために、HTMLに表示する際は `htmlspecialchars` を使用します。このような無害化・エスケープ処理を「`htmlspecialchars`で、特殊文字をHTMLエンティティに変換する」などと表現します。
   - **HTMLエンティティへの変換処理:**  
     HTMLエンティティに変換するための`htmlspecialchars`ですが、今回、この第二引数に`ENT_QUOTES`を使っています。この第二引数は特殊文字をどのように扱うのか？という指示に相当します。今回のように`ENT_QUOTES`を指定すると、特殊文字のうちシングルクォーテーションとダブルクォーテーションも変換対象に含めるようになります。
   - **ファイルアップロード:**  
...
これまでのHTML要素からの入力例と同じように、**`action属性`**で実行するPHPのファイル名を指定します。その意味では同じなのですが、実行するPHPのコードの中にSQLへのアクセスデータを入れるという点で違いがあります。  
...
1. **セキュリティ:**  
   ...
   ※今回は実装していませんが、機密情報はコードに直接記述（ハードコート）せず`.env`ファイルなどを利用して管理しましょう。
